...
export async function POST(req: Request): Promise<NextResponse> {
  const apiUrl = process.env.UPSCALE_API_URL;
  const apiKey = process.env.UPSCALE_API_KEY;
  if (!apiUrl || !apiKey) {
    return NextResponse.json(
      {
        error:
          "Upscaling is not configured. Set UPSCALE_API_URL and UPSCALE_API_KEY in .env.local.",
      },
      { status: 503 }
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Upscaling failed unexpectedly." },
      { status: 500 }
    );
  }
}
